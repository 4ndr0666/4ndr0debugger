...
import { SaveIcon, ImportIcon, BoltIcon, ChatIcon, HistoryIcon, EyeIcon, EyeOffIcon, DocsIcon, FolderIcon, ReportIcon, TargetIcon, SkullIcon, CrosshairsIcon, ToastInfoIcon as InfoIcon, CogIcon } from './Icons.tsx';
...
export const RadialMenu: React.FC<RadialMenuProps> = (props) => {
    ...
    const menuItems: MenuItem[] = useMemo(() => [
        { type: 'item', label: 'System', description: 'Configure system feature flags.', icon: <CogIcon />, action: props.onOpenFeatureFlagsModal, disabled: anyLoading },
        ...
        { type: 'item', label: 'Threat Vector', description: 'Analyze a URL for potential attack surfaces.', icon: <CrosshairsIcon />, action: props.onOpenThreatVectorModal, disabled: anyLoading, flag: featureFlags.red_team_tools },
        { type: 'item', label: 'Live Recon', description: 'Generate a script to capture network traffic on a target page.', icon: <TargetIcon />, action: props.onOpenReconModal, disabled: anyLoading, flag: featureFlags.red_team_tools },
        { type: 'item', label: 'Exploit Stager', description: 'Generate payload delivery commands for a known vulnerability.', icon: <SkullIcon />, action: props.onOpenExploitStagerModal, disabled: anyLoading, flag: featureFlags.red_team_tools },
        { type: 'item', label: 'Adv. Report', description: 'Generate a bug bounty report from captured recon data.', icon: <ReportIcon />, action: props.onOpenReportGenerator, disabled: anyLoading, flag: featureFlags.red_team_tools },
        ...
        { type: 'divider', label: 'Tools', flag: featureFlags.red_team_tools },
        ...
        { type: 'item', label: 'Project Files', description: 'Manage uploaded files for context.', icon: <FolderIcon />, action: props.onOpenProjectFilesModal, disabled: anyLoading, flag: featureFlags.multi_file_context },
    ...
    ], [props, anyLoading, isToolsEnabled, featureFlags]);

    const visibleItems = menuItems.filter(item => item.flag === undefined || item.flag === true);
